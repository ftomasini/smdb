...
var test_{{$id}} = function(r){
	var dt = dt_{{$id}}, rs = dt.getRecordSet();
	var coldefs = dt.getColumnSet().getDefinitions(), params = {};
	var d, i;
	for (i in coldefs){
		d = r.getData(coldefs[i].key);
		params[coldefs[i].key] = (typeof d) == "undefined" ? "" : d;
	}
	$.get("index.php?page=configure&func={{$id}}-test", params,
		function(data,result){
			var j;
			if (result=="success"){
				eval("var res="+data);
...
var delete_{{$id}} = function(r){
	...
	for (i in coldefs){
		d = r.getData(coldefs[i].key);
		params[coldefs[i].key] = (typeof r) == "undefined" ? "" : d;
	}
...
{{/if}}
function node_rowClick(oArgs){
	selected_instance = this.getRecord(oArgs.target);
}
dt_{{$id}}.subscribe("rowClickEvent", node_rowClick);
{{/if}}
